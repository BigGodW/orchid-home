<template>
  <div class="w-full overflow-auto p-2 md:p-4">
    <!-- 搜索区域（适配移动端） -->
    <div class="mb-4 md:mb-6 flex flex-col sm:flex-row items-start sm:items-center gap-3 w-full">
      <input
        v-model="searchKeyword"
        type="text"
        placeholder="输入兰花品种名称搜索..."
        class="px-4 py-2 border rounded-lg w-full sm:w-64 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm"
        @input="handleSearch"
      />
      <div class="flex gap-2 w-full sm:w-auto">
        <button
          @click="clearSearch"
          class="px-3 py-2 bg-gray-200 rounded-lg hover:bg-gray-300 transition-colors text-sm w-full sm:w-auto"
        >
          清空搜索
        </button>
        <span v-if="searchedBlocks.length > 0" class="text-sm text-gray-600 px-2 py-2">
    ...
    <!-- 入库表单 -->
    <OrchidInboundForm
      :show="showInboundForm"
      :block="selectedBlock"
      @close="showInboundForm = false"
